...
export default function Home() {
  ...
  async function uploadFiling(file: File) {
    ...
    try { const response = await fetch(`${API}/v1/documents`, { method: "POST", body: data }); if (!response.ok) throw new Error((await response.json()).detail); const result = await response.json(); await loadTopics(); if (result.chat_topic) setActiveTopic(result.chat_topic); }
