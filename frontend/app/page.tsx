...
export default function Home() {
  ...
  if (!token) return <main className="auth-page"><section className="auth-card"><h1>Analyst Copilot</h1><p>Sign in to access your private filing workspace.</p>{error && <p className="error" role="alert">{error}</p>}<form className="auth-form" onSubmit={signIn}><label>Email<input required type="email" value={email} onChange={e => setEmail(e.target.value)} autoComplete="email"/></label><label>Password<input required type="password" value={password} onChange={e => setPassword(e.target.value)} autoComplete="current-password"/></label><button disabled={signingIn}>{signingIn ? "Signing in…" : "Sign in"}</button></form></section></main>;
